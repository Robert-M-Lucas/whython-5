...
impl Type for BoolType {
    ...
    fn allocate_variable(&mut self, stack: &mut StackSizes, program_memory: &mut MemoryManager, to_assign: Option<&Literal>) -> Result<(), String> {
        if self.address.is_some() { warn(format!("Allocating {} when it already has a memory address", self.get_type_symbol()).as_str()) }
    ...
    fn get_constant(&self, literal: &Literal) -> Result<Address, String> {
        match literal {
            ...
            other => {
                create_literal_not_impl_error(other, self.get_type_symbol())
            }
        }
