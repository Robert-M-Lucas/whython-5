...
impl LineHandler for PrintDumpLine {
    fn process_line(
        ...
        if line.len() != 2 {
            return ProcessingResult::Failure("PrintDump must be followed by a variable".to_string());
        }
        ...
        let variable = match &line[1] {
            Symbol::Name(name) => {
                match &q!(block_coordinator.get_variable(name.as_str())).reference {
                    ...
                    _ => return ProcessingResult::Failure("PrintDump must be followed by a variable".to_string())
                }
        ...
        PrintDumpInstruction::new_alloc(memory_manager, variable);
